...
export class CoursService {
  ...
  async getList(ecole: any, eleve:any, offset:number = 0): Promise<Observable<any>>{
    ...
    return this.api.postData("action_mobile.php", {"action": "lecons_json", "telephone": telephone, ecole:ecole, eleve:eleve, offset:offset});
  }
  ...
  async getListCoachingFree(type: string = "", coach: any, offset:number = 0): Promise<any>{
    let telephone = await this.userData.getTelephone();
    let userId = await this.userData.getId();
    return this.api.postData("action_mobile.php", {"action": "lecons_json", acces: "all", coach:coach, "telephone": telephone, eleve:userId, coaching:'oui', type:type, offset:offset}).toPromise();
  }
